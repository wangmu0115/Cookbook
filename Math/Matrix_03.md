...
矩阵 $\boldsymbol{B}$ 必须与 $\boldsymbol{A}$ 相同，并采用相同的分块法切割 $\boldsymbol{A}$ 和 $\boldsymbol{B}$。
$$
...
$\boldsymbol{A}$ 和 $\boldsymbol{B}$ 相乘时，首先保证 $\boldsymbol{A}$ 的列数等于 $\boldsymbol{B}$ 的行数，$\boldsymbol{A}$ 和 $\boldsymbol{B}$ 分块时，保证 $\boldsymbol{A}$ 的每一个子块矩阵的列数分别等于对应位置 $\boldsymbol{A}$​ 的每个子块的行数。
$$
...
### 7. $\boldsymbol{A}$ 上下切，$\boldsymbol{B}$ 左右切
...
### 8. $\boldsymbol{A}$ 左右切，$\boldsymbol{B}$ 上下切
...
### 9. $\boldsymbol{A}$ 和 $\boldsymbol{B}$ 都上下左右分块
...
**克罗内克积**也叫矩阵张量积，是两个任意大小矩阵矩阵之间的运算，运算符是 $\otimes$。

矩阵 $\boldsymbol{A}$ 的形状为 $n \times m$，$\boldsymbol{B}$ 的形状为 $p \times q$，$\boldsymbol{A} \otimes \boldsymbol{B}$ 的形状为 $np \times mq$。
$$
